## 台站值班客户端挂机程序
- 广西广播电视技术中心的台站值班客户端.exe软件在运行一段时间（20+小时）后会出现卡死现象，并且弹出多个程序错误窗口，导致无法听到各系统的告警声音，必须手动结束值班客户端进程，再重新打开台站值班客户端软件。
- 因台站值班客户端多在WindowsXP/Win7使用，所以选用了更为轻便的C#进行开发本程序的界面和功能。
- 在24小时无人值班过程中，让电脑自动重启台站值班客户端.exe软件，“挂机”就相当于每天手动重启一次台站值班客户端.exe软件，避免客户端“死机”情况的发生。
- 另外加入了启动、关闭、重启台站值班客户端软件的功能，也可以在该挂机软件界面上选择启动台站值班客户端时所显示的界面。
- 注意：目前用户名和密码都还为固定字串，在程序中修改用户名和密码后要重新生成解决方案才可使用。
---
...
## 使用  
运行环境：Win7 x64（推荐）；.NET Framework 4.0及以上  
1.直接拷贝bin文件夹下的整个Release文件夹到电脑  
2.打开 MD5校验器.exe，浏览电脑文件夹，找到用于运行挂机的【台站值班客户端.exe】，计算出 md5、sha1、crc32  
3.文本编辑器打开 BOT_Client.exe.Config，复制上一步骤得到的 md5、sha1、crc32 来修改Config文件里对应的value，修改完成后保存退出  
4.运行BOT_Client.exe程序  

注1：目前请不要在挂机程序运行的时候修改 BOT_Client.exe.Config文件，不然会出现程序卡死，之后的版本会修复此问题  
注2：更换台站使用时，必须要更换对应台站的用户名/密码，请根据网上的键盘键值表修改并重新用VS发布exe程序。之后的版本会增加在外部设置账号密码的功能
（目前本程序在登陆客户端后自动输入的用户名密码均为：bs247t）
...
### 更新：
	...
	2.启动/重启客户端：加入文件判断（md5/sha1/crc32），所选文件是否为设定指向的台站值班客户端.exe
	3.选择文件：修改初始路径为F盘根目录
### 待加入的功能：
	1.增加用户名和密码的设置项，使其能够在外部修改
	2.增加自动重启时间的设置项，使其能够在外部修改
### 待修复：
	1.挂机程序正在运行，修改配置文件 BOT_Client.exe.Config 内容保存后，挂机程序卡死
---
### 2017-07-07
- 程序版本：V1.10
- 台站客户端版本：2017.04.28
...
### 读取和保存配置功能：
	1.每次启动时会读取配置文件“BOT_Client.exe.config”中的各个配置项目：
		...
		autoReatart：每天02:30自动重启值班客户端，勾选为true，不勾选为false
		silent：关闭“信号源系统”声音
...
### 桌面分辨率判断功能
	...
	其余分辨率当做4:3处理。
### 用到的辅助类
	HashHelper: 计算文件的MD5、SHA1、CRC32的值
	InputHelper：模拟鼠标和键盘的单个动作
